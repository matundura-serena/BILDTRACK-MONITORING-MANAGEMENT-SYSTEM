...
const TaskDistributionChart = ({ data, title = "Task Distribution", chartWidth }) => {
  if (!data || !data.chart || !data.chart.datasets || data.chart.datasets.length === 0) {
    return (
      <View style={styles.emptyContainer}>
        <Text style={styles.emptyText}>No task distribution data available</Text>
      </View>
    );
  }
      ...
      {summary && (
        <View style={styles.summaryContainer}>
          <View style={styles.summaryItem}>
            <Text style={styles.summaryValue}>{summary.total_tasks || 0}</Text>
            <Text style={styles.summaryLabel}>Total Tasks</Text>
          </View>
          <View style={styles.summaryItem}>
            <Text style={styles.summaryValue}>
              {summary.total_tasks > 0 ? Math.round((summary.completed / summary.total_tasks) * 100) : 0}%
            </Text>
